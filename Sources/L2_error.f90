subroutine L2_error(cal_time)
  ...
  integer(kind=ik):: i, imaxr, imaxz, imaxu, imaxv, imaxT, imaxcp, imaxgamma, imaxp, imaxc, write_node
  integer(kind=ik):: imaxRsi, imaxReta, imaxRu, imaxRv, imaxRt, imaxRcp, imaxRgamma, imaxRp, imaxRc
  real(kind=rk):: dr(NTN), dz(NTN), du(NTN), dv(NTN), dTemp(NTN), dcp(NTN), dgamma(NTN), dpress(NTN), dc(NTN), &   
       drmax, dzmax, dumax, dvmax, dTmax, dcpmax, dgammamax, dpmax, dcmax      !dTemp dstinct from dt in data
  real(kind=rk):: Rsi(NTN), Reta(NTN), Ru(NTN), Rv(NTN), Rt(NTN), Rcp(NTN), Rgamma(NTN), Rp(NTN), Rc(NTN), &
       Rsimax, Retamax, Rumax, Rvmax, Rtmax, Rcpmax, Rgammamax, Rpmax, Rcmax
  ...
  do i = 1, NTN   !?change split_sol accordingly
     ...
     if(s_mode.eq.0) then
        if(initial_vapor_solved.eq.1) then
           ...
           if(VN(i).eq.5 .and. solve_T.eq.1) error2 = error2 + ( dsol(NOPP(i)+NTs)/Tmax )**2
        end if
...
if(diverge.eq.1) then
     ...
     if(s_mode.eq.0) then
        if(VN(i).eq.0 .or. VN(i).eq.2) then
           ...
           if(solve_cp.eq.1) then
              ...
              if(surf_adsp.eq.1 .and. (BCflagN(i,3).eq.1 .or. BCflagN(i,3).eq.3) ) &
                   dgamma(i) = abs( dsol( NOPP(i)+MDF(i)-1 ) )
           end if !solve_cp
        ...
        if( (VN(i).eq.1 .or. VN(i).eq.2) ) then
           ...
           if( BCflagN(i,2).eq.1 .and. solve_T.eq.1)  dTemp(i) = abs( dsol( NOPP(i) + NT ) / Tmax )
        end if
  ...
  if (step.eq.1) then
     ...
     write(10, '(A)') 'variables = "dr", "dz", "du", "dv", "dT", "dcp", "dgamma", "dp", "dc" '
  else
  ...
  write(10,'(9es12.5)') drmax, dzmax, dumax, dvmax, dTmax, dcpmax, dgammamax, dpmax, dcmax
  if(write_node.eq.1) write(10,'(9i12)') imaxr, imaxz, imaxu, imaxv, imaxT, imaxcp, imaxgamma, imaxp, imaxc
...
!-------------------------calculate Res error seperately, Linf norm--------------------
  ...
  do i = 1, NTN, 1
     ...
     if(VN(i).eq.0 .or. VN(i).eq.2) then
        ...
        if(solve_cp.eq.1) then
           ...
           if(surf_adsp.eq.1 .and. (BCflagN(i,3).eq.1 .or. BCflagN(i,3).eq.3) ) &
                Rgamma(i) = abs( load_dum( NOPP(i)+MDF(i)-1 ) )
        end if !solve_cp
     ...
     if( (VN(i).eq.1 .or. VN(i).eq.2) .and. s_mode.eq.0) then
        ...
        if( BCflagN(i,2).eq.1 .and. solve_T.eq.1)  Rt(i) = abs( load_dum( NOPP(i) + NT ) )
     end if
     if (VN(i).eq.5 .and. s_mode.eq.0 .and. solve_T.eq.1) then
  ...
  if (timestep.eq.0 .and. step.eq.1 .and. ( size_function_change.eq.1 .or. read_coordinate_value.eq.1 ) ) then
     ...
     write(12, '(A)') 'variables = "Rsi", "Reta", "Ru", "Rv", "Rt", "Rcp", "Rgamma", "Rp", "Rc" '
  else
  ...
  write(12,'(9es12.5)') Rsimax, Retamax, Rumax, Rvmax, Rtmax, Rcpmax, Rgammamax, Rpmax, Rcmax
  if(write_node.eq.1) write(12,'(9i12)') imaxRsi, imaxReta, imaxRu, imaxRv, imaxRt, imaxRcp, imaxRgamma, imaxRp, imaxRc
