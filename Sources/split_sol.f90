  ...
  do i = 1, NTN
     ...
     if( BCflagN(i,2).ne.0 ) then
        ...
        if(s_mode.eq.0 .and. VN(i).ne.1) then
           ...
           if(substrate.eq.0.0_rk .and. solve_T.eq.1) sol( NOPP(i) + NT ) = 0.0_rk 
        end if
     end if
  ...
  do i = 1, NTN, 1
     ...
     if(s_mode.eq.0) then
        if(VN(i).eq.0 .or. VN(i).eq.2 .or. BCflagN(i,2).eq.1) then
           if(solve_T.eq.1) Tsol(i) = sol( NOPP(i) + NT ) 
           if( VN(i).eq.0 .or. VN(i).eq.2 ) then           
              usol(i) = sol( NOPP(i) + Nu ) 
              vsol(i) = sol( NOPP(i) + Nv ) 
              if(solve_cp.eq.1) cpsol(i) = sol( NOPP(i) + Ncp ) 
              if ( PN(i).eq.1 )     psol(i) = sol( NOPP(i) + Np ) 
           end if
        end if
        if(VN(i).eq.5 .and. solve_T.eq.1) Tsol(i) = sol( NOPP(i) + NTs )
