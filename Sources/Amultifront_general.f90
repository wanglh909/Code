...
module front_mod
  ...
  use data, only: rmax, zmax, umax, vmax, Tmax, cpmax, pmax, cmax, &
       ...
       Nr, Nz, NT, Nu, Nv, Ncp, Np, NTs, solve_T
  use omp_lib
       ...
       if(s_mode.eq.0) then
          if(initial_vapor_solved.eq.1) then
             if( VN(i).eq.0 .or. VN(i).eq.2 .or. BCflagN(i,2).eq.1) then
                if(solve_T.eq.1) L2res = L2res + ( load_dum(NOPP(i)+NT)/Tmax )**2
             ...
             if(VN(i).eq.5 .and. solve_T.eq.1) L2res = L2res + ( load_dum(NOPP(i)+NTs)/Tmax )**2
          end if
